...
## 13. 결정 완료 항목 (Locked for v1)

- 상태코드 표준 매핑: `CLOSE > OPEN > UPDATE > OTHER` 우선순위로 고정한다.
  - `CLOSE`: 폐업/취소/말소 계열 상태코드
  - `OPEN`: 영업/정상/개시 계열 상태코드
  - `UPDATE`: 정정/변경/갱신 계열 상태코드
  - `OTHER`: 위 규칙으로 분류되지 않는 상태코드
- 브랜드 유사 매칭 임계값 기본값: `0.88`로 확정한다.
  - `>= 0.93`: 자동 확정
  - `0.88 <= score < 0.93`: 운영 검토 큐로 이관
  - `< 0.88`: 미매칭 처리
- 소스 간 충돌 우선순위 규칙:
  - 사실 이벤트(개업/폐업) 우선순위: `data.go.kr > legacy/local backup`
  - 브랜드/가맹 메타데이터 보강: `KFTC` 사용(사실 이벤트 덮어쓰기 금지)
  - 동일 키 충돌 타이브레이커: `updated_at_source 최신 > source priority > ingested_at 최신`
- 분류/충돌 처리 로그 규칙:
  - 충돌 해결 결과는 `winner_source`, `loser_source`, `resolution_reason`를 감사 로그에 남긴다.
  - 미분류(`OTHER`) 비율 2% 초과 시 경보를 발생시킨다.

## 13.1 웹 테스트 버전 운영 기준

- v1 공개 전 `web-test` 환경을 운영하여 기능/성능/데이터 품질을 사전 검증한다.
- 테스트 배포는 운영과 동일한 API 계약(`/api/v1`)과 집계 규칙을 사용한다.
- 테스트 환경 도메인은 CORS 허용 목록에 명시적으로 추가한다.
- 테스트 배포 승인 기준:
  - AC-01 ~ AC-07 모두 충족
  - 최근 7일 스케줄 실행 성공률 99% 이상
  - 최근 7일 데이터 최신성 지표 95% 이상
